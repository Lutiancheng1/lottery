...
class CanadaDataManager:
    ...
    def get_fanshu(self, t, o):
        """
        根据新站逻辑推导的番数逻辑: (拾*10 + 个) % 4 (余数为0时记为4)
        """
        try:
            val = (int(t) * 10 + int(o)) % 4
    ...
    def format_data_line(self, data: Dict) -> str:
        ...
        f_val = self.get_fanshu(int(s), int(g))
    ...
    def get_realtime_data(self) -> Optional[Dict]:
        ...
        try:
            ...
            init_res = self.session.get(init_url, params={"awaken": 180}, headers=self.headers, timeout=10)
            init_data = init_res.json()
            
            # 2. 获取最新开奖结果 (refresh)
            open_url = f"{self.base_url}/refresh"
            # refresh 是 POST 但没有 body
            ...
            user_res = self.session.get(user_url, headers=self.headers, timeout=10)
            ...
            if init_data.get('code') == 700 and user_data.get('code') == 703:
                ...
                return {
                    'server_at': now,
                    'user': {
                        'CM_surplus': float(credit.get('SURP', 0)) / 10000.0
                    },
                    'n_period': {
                        'period_no': lottry.get('ltn'),
                        'finish_at': now + remaining,
                        'period_status': 1
                    },
                    'p_period': {
                        'period_no': last_open.get('LTTNUM'),
                        'number_overt': last_open.get('NUMS', '').replace(',', '')
                    }
                }
            else:
    ...
    def fetch_remote_history(self, page=1, limit=300) -> Optional[Dict]:
        """获取远程历史记录 (适配 s4.pd988.xyz)"""
        if not self.cookie:
            return None
        try:
            url = f"{self.base_url}/page/lottery/showHistoryLottery"
            # 如果没有指定日期，默认查当天的？这里先按 YYYYMMDD 格式处理
            import datetime
            today_str = datetime.datetime.now().strftime("%Y%m%d")
            payload = {
                "paramMap.pageNum": page,
                "paramMap.pageSize": limit,
                "paramMap.lttnum": today_str
            }
            # 注意：新站是 form-urlencoded
            ...
            if res_json.get('code') == 700:
                ...
                for row in new_rows:
                    ...
                    converted_data.append({
                        ...
                        'lhh': self.get_lhh(b, g), 
                        'fan': f"{self.get_fanshu(t, o)}番",
                        'fan_sum': self.get_fanshu(t, o)
                    })
                        
                # 过滤掉尚未开奖的期号
                filtered_data = [d for d in converted_data if d.get('number_overt') and d.get('number_overt') != "000"]
