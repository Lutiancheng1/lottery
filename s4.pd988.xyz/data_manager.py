...
class CanadaDataManager:
    ...
    def __init__(self, data_dir="Data"):
        ...
        self.base_url = "https://s4.pd988.xyz"
        self.headers = {
        ...
        self.SCALE = 10000 # 金额缩放比例: 新站 1=10000 (支持到0.0001但通常为0.001)
    ...
    def get_realtime_data(self) -> Optional[Dict]:
        """获取实时游戏数据 (适配 s4.pd988.xyz)"""
        if not self.cookie:
            return None
        try:
            # 1. 获取倒计时和当前期号
            # https://s4.pd988.xyz/initHome?awaken=180
            init_url = f"{self.base_url}/initHome"
            params = {"awaken": 180, "_": int(time.time() * 1000)}
            init_res = self.session.get(init_url, params=params, headers=self.headers, timeout=10)
            init_data = init_res.json()
            
            # 2. 获取最新开奖结果 (Previous Period)
            # https://s4.pd988.xyz/refresh
            open_url = f"{self.base_url}/refresh"
            open_res = self.session.post(open_url, data={}, headers=self.headers, timeout=10)
            open_data = open_res.json()
            
            # 3. 获取余额
            # https://s4.pd988.xyz/initUserInfo
            user_url = f"{self.base_url}/initUserInfo"
            user_res = self.session.get(user_url, params={"_": int(time.time() * 1000)}, headers=self.headers, timeout=10)
            user_data = user_res.json()

            if init_data.get('code') == 700 and user_data.get('code') == 703:
                # print("✅ API 数据获取成功")
                pass
            else:
                print(f"⚠️ API 返回错误: init={init_data.get('code')}, user={user_data.get('code')}")
                return None
            
            lottry = init_data.get('lottry', {})
            credit = user_data.get('credit', {})
            last_open = open_data.get('data', {}) if open_data.get('code') == 700 else {}
            
            now = time.time()
            # remainingTime 单位是毫秒
            try:
                remaining = float(lottry.get('remainTime', 0)) / 1000.0
            ...
            return {
                'server_at': now,
                'user': {
                    'CM_surplus': float(credit.get('SURP', 0)) / self.SCALE # 转换为实际可用金额
                },
                'n_period': {
                    'period_no': lottry.get('lttnum'),
                    'finish_at': now + remaining,
                    'period_status': 1 
                },
                'p_period': {
                    'period_no': last_open.get('LTTNUM'),
                    'number_overt': last_open.get('NUMS') # 这里的 NUMS 是 "3,1,1"
                }
            }
        except Exception as e:
            print(f"❌ 获取实时数据失败: {e}")
    ...
    def fetch_remote_history(self, page=1, limit=100) -> Optional[Dict]:
        ...
        try:
            ...
            if res_json.get('code') == 700:
                new_rows = res_json.get('pageInfo', {}).get('list', [])
                converted_data = []
                for row in new_rows:
                    hundred = row.get('HUNDRED', '0')
                    ten = row.get('TEN', '0')
                    one = row.get('ONE', '0')
                    
                    # 只有当有效时才处理
                    if row.get('ISVALID') != 1:
                        continue
                        
                    open_num = f"{hundred},{ten},{one}"
                    b, s, g = int(hundred), int(ten), int(one)
                    result_sum = b + s + g
                    
                    converted_data.append({
                        'overt_at': row.get('LTTTIME'),
                        'period_no': row.get('LTTNUM'),
                        'b': b,
                        's': s,
                        'g': g,
        ...
        try:
            ...
            if res_json.get('code') == 700:
                ...
                for row in rows:
                    ...
                    hundred, ten, one = row.get('HUNDRED', '0'), row.get('TEN', '0'), row.get('ONE', '0')
                    open_num = f"{hundred},{ten},{one}"
                    b, s, g = int(hundred), int(ten), int(one)
                    result_sum = b + s + g
    ...
    def fetch_single_period(self, period_no) -> Optional[Dict]:
        """获取指定单个期号的数据 (通过日期反推, 适配 s4.pd988.xyz)"""
        if not self.cookie:
            return None
        ...
        try:
            ...
            # 直接按期号搜
            payload = {"paramMap.lttnum": str(target_no)}
            resp = self.session.post(url, data=payload, headers=self.headers, timeout=10)
            res_json = resp.json()
