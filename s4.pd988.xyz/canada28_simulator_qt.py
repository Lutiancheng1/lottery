                ...
                for attempt in range(3):
                    try:
                        ...
                        if batch_count == 1 and attempt == 0:
                            self.log_signal.emit(f"🚀 发送下单请求: 期号={self.period}, 总额={total_money:.2f}")
                        else:
...
class Canada28Simulator(QMainWindow):
    ...
    def create_control_tabs(self):
        ...
        self.spin_stop_loss.setRange(-1000000, 1000000)
        ...
        if os.path.exists(config_path):
            try:
                with open(config_path, "r") as f:
                    ...
                    # 禁用参数变更信号，防止加载时触发日志和重复保存
                    self.param_initialized = False
                    
                    # self.last_numbers_file 已移除
                    
                    # 加载基础参数
                    if "payout" in config: self.spin_payout.setValue(config["payout"])
                    if "unit_bet" in config: self.spin_unit_bet.setValue(config["unit_bet"])
                    if "max_bet_enabled" in config: self.chk_max_unit_bet.setChecked(config["max_bet_enabled"])
                    if "max_bet_val" in config: self.spin_max_unit_bet.setValue(config["max_bet_val"])
                    if "low_balance_enabled" in config: self.chk_low_balance.setChecked(config["low_balance_enabled"])
                    if "low_balance_val" in config: self.spin_low_balance.setValue(config["low_balance_val"])
                    
                    # 加载策略参数
                    if "increase_rate" in config: self.spin_increase_rate.setValue(config["increase_rate"])
                    if "increase_fixed" in config: self.spin_increase_fixed.setValue(config["increase_fixed"])
                    if "decrease_rate" in config: self.spin_decrease_rate.setValue(config["decrease_rate"])
                    
                    # 加载止盈止损
                    if "take_profit_enabled" in config: self.chk_take_profit.setChecked(config["take_profit_enabled"])
                    if "take_profit_val" in config: self.spin_take_profit.setValue(config["take_profit_val"])
                    if "stop_loss_enabled" in config: self.chk_stop_loss.setChecked(config["stop_loss_enabled"])
                    if "stop_loss_val" in config: self.spin_stop_loss.setValue(config["stop_loss_val"])

                    if "my_numbers" in config and config["my_numbers"]:
    ...
    def save_config(self):
        ...
        try:
            config = {
                ...
                "increase_rate": self.spin_increase_rate.value(),
                "increase_fixed": self.spin_increase_fixed.value(),
                "decrease_rate": self.spin_decrease_rate.value(),
                "take_profit_enabled": self.chk_take_profit.isChecked(),
                "take_profit_val": self.spin_take_profit.value(),
                "stop_loss_enabled": self.chk_stop_loss.isChecked(),
                "stop_loss_val": self.spin_stop_loss.value(),
                "max_bet_val": self.spin_max_unit_bet.value(),
                "low_balance_enabled": self.chk_low_balance.isChecked(),
                "low_balance_val": self.spin_low_balance.value(),
                "my_numbers": list(self.my_numbers) if isinstance(self.my_numbers, (set, list)) else [] # 转换为列表
            }
            config_path = self.get_config_path("config.json")
            ...
            print(f"❌ 保存配置失败: {e}")
            QMessageBox.critical(self, "配置保存失败", error_msg)
    ...
    def import_from_txt(self, filepath=None, silent=False):
        ...
        if filepath:
            try:
                ...
                # 保存配置
                # self.last_numbers_file = filepath (已移除)
                self.save_config()
    ...
    def place_real_bet(self, period, unit_bet):
        ...
        self.log_run(f"🚀 准备下单: 期号={period}, 总额={total_money:.2f}")
    ...
    def on_param_changed(self, param_name, value):
        ...
        # 记录参数日志 (如果是浮点数则保留2位小数)
        display_value = value
        if isinstance(value, float):
            display_value = round(value, 2)
            
        self.log_run(f"⚙️ 参数变更: {param_name} = {display_value}")
        ...
        try:
            ...
            if last_period and last_result:
                ...
                if last_period != self.last_processed_period:
                    ...
                    # 3. 刷新表格（此时可能还没同步完，但没关系，同步完后 on_sync_finished 会再次刷新）
                    self.update_history_table()
