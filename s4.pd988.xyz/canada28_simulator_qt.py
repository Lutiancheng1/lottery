...
class AccountSyncWorker(QThread):
    ...
    def run(self):
        try:
            ...
            if response.status_code == 200:
                ...
                if res_json.get("code") == 700:
                    ...
                    for item in data_list:
                        ...
                        try:
                            total_bet_val = float(item.get("amounts", 0) or 0) / 10000.0
                            win_amount_val = float(item.get("bonuss", 0) or 0) / 10000.0
                            profit_val = float(item.get("yk", 0) or 0) / 10000.0
                        except (ValueError, TypeError):
                            total_bet_val, win_amount_val, profit_val = 0.0, 0.0, 0.0
            ...
            # 第二阶段：获取最近20期的详细明细 (queryOrderDetail)
            ...
            for idx, p_no in enumerate(recent_periods):
                try:
                    ...
                    detail_payload = {"paramMap.lttnum": str(p_no)}
                    ...
                    if detail_res.status_code == 200:
                        ...
                        if detail_json.get("code") == 700:
                            ...
                            if orders:
                                t_bet, t_prize, u_bet = 0.0, 0.0, 0.0
                                for o in orders:
                                    t_bet += float(o.get("amount", 0)) / 10000.0
                                    t_prize += float(o.get("bonus", 0)) / 10000.0
                                    if u_bet == 0: u_bet = float(o.get("amount", 0)) / 10000.0
...
class BettingWorker(QThread):
    ...
    def run(self):
        try:
            ...
            # 构造 betNoList (新站格式: [{"nm": "001", "am": 1000}])
            betNoList = []
            for num in self.my_numbers:
                # 网页显示 0.1 元，API 传 1000。所以 1 元对应 10000。
                betNoList.append({"nm": str(num), "am": int(self.unit_bet * 10000)})
            ...
            payload = {
                "paramMap.nms": json.dumps(betNoList),
                "paramMap.bw": 4,
            ...
            if response.status_code == 200:
                res_json = response.json()
                success_count = res_json.get("scc", 0)
                
                if success_count > 0:
                    msg = f"下单成功 ({success_count}注)"
                    self.success_signal.emit(self.period, msg)
                else:
                    fail_count = res_json.get("fdc", 0)
                    error_msg = res_json.get('msg', f'下单失败 (失败:{fail_count}注)')
        ...
        try:
            ...
            if self.is_running:
                ...
                if is_market_open and current_period:
                    # === 修复竞态条件：确保上一期已处理完成再下单 ===
                    # 逻辑：如果当前是第 N 期，我们需要确认第 N-1 期已经同步并处理过（更新了注码）
                    try:
                        curr_p_int = int(current_period)
                        prev_p_str = str(curr_p_int - 1)
                        
                        # 如果上一期还没处理完，且上一期确实存在于我们的历史视野中
                        # 我们等待数据同步线程完成工作
                        if not hasattr(self, 'processed_periods') or prev_p_str not in self.processed_periods:
                            # 检查本地数据库是否有这一期，如果没有，说明同步还没到
                            latest_local = self.data_manager.get_local_latest()
                            if latest_local and str(latest_local.get('period_no')) != prev_p_str:
                                # 只有在距离封盘还有足够时间时才等待，否则为了防止漏单强制下单
                                if diff > 10: # 还有10秒以上，可以等同步
                                    if not hasattr(self, '_last_wait_sync_log') or self._last_wait_sync_log != current_period:
                                        self.log_run(f"⏳ 等待第 {prev_p_str} 期数据同步以更新注码...")
                                        self._last_wait_sync_log = current_period
                                    return 
                    except:
                        pass
    ...
    def show_order_details(self, period_no):
        ...
        try:
            ...
            if res_json.get("code") != 700:
                ...
                QMessageBox.information(self, f"第 {period_no} 期详情", f"未查询到该期下单详情: {res_json.get('msg')}")
                return
            ...
            detail_text += "<tr><th>号码</th><th>下单金额</th><th>赔率</th><th>中奖金额</th><th>时间</th><th>状态</th></tr>"
            ...
            for o in orders:
                num = o.get("num", "--")
                amt = float(o.get("amount", 0)) / 10000.0
                odds = float(o.get("odds", 0)) / 10000.0
                prize = float(o.get("bonus", 0)) / 10000.0
                b_time = o.get("bt", "--")
                status = "中奖" if float(o.get("bonus", 0)) > 0 else "未中"
                # 新站 API: status 为 "timeOut" 表示未开奖/待结算，"normal" 表示已结算
                ...
                color = "red" if prize > 0 else "black"
                detail_text += f"<tr><td>{num}</td><td>{amt:.2f}</td><td>{odds:.2f}</td><td style='color:{color}'>{prize:.2f}</td><td>{b_time}</td><td>{status}</td></tr>"
